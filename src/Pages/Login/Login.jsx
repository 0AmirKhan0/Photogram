...
const fetchUserData = async (username) => {
  return getReq(`/users?username=${username}`)
    .then((users) => users[0])
    .catch();
};
...
export default function Login() {
  ...
  const handleSignup = (e) => {
    e.preventDefault();
    fetchUserData(singupForm.username)
      .then((user) => {
        if (user) {
  ...
  return (
    <>
      {loading ? (
        ...
        <div
          className={`container-sign ${mode === MODES.singup ? "active" : ""}`}
          id="login-container"
        >
          ...
          <div className="toggle-container">
            <div className="toggle">
              <div className="toggle-panel toggle-left">
                ...
                <button
                  onClick={(e) => handleMode(MODES.signin)}
                  className="hidden"
                  id="login"
                >
                  Sign In
              ...
              <div className="toggle-panel toggle-right">
                <h1>Hello, Friend!</h1>
                <button
                  onClick={(e) => handleMode(MODES.singup)}
                  className="hidden"
                  id="register"
                >
                  Sign Up
