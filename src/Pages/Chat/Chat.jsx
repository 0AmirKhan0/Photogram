...
export default function Chat() {
  ...
  useLayoutEffect(() => {
    getReq(`/chats?id=${id}`)
      .then((chats) => setChat(chats[0]))
      .catch();
  }, []);
  useEffect(() => {
    if (chat && user) {
      //   console.log("test");
      //   getReq(`/messages?chatId=${chat.id}&_sort=created_time&_order=asc`)
      getReq(`/messages?chatId=${chat.id}`)
        .then((messages) => {
    ...
    if (chat?.user1Id === user.id) {
      getReq(`/users?id=${chat.user2Id}`)
        .then((users) => setSecondUser(users[0]))
        .catch();
    } else {
      getReq(`/users?id=${chat?.user1Id}`)
        .then((users) => setSecondUser(users[0]))
        .catch();
    }
  }, [chat, user]);
      ...
      <section className="container chat-section">
          ...
          {messages ? (
            messages.map((message, index) => {
              if (message?.senderId === user.id) {
                return (
                  <li key={message.id} className={"friend-user"}>
                    <p>{message.content}</p>
                ...
                return (
                  <li>
                    {messages[index + 1]?.senderId === secondUser?.id ? (
                      <p className="fix-message-padding">{message.content}</p>
                    ) : (
                      <>
                        <Link to={`/user/${secondUser?.id}`}>
                          <img
        ...
        <div className="send-message-container">
          ...
          <button className="send-message" onClick={sendMessage}>
            ...
            >
              <path d="m0 0 20 10-20 10zm0 8v4l10-2z" />
            </svg>
          </button>
        </div>
      </section>
